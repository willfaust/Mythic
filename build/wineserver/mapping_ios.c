...
#ifdef WINE_IOS
    /* iOS quirk: mmap(PROT_WRITE, MAP_SHARED) on file fd silently grants
     * read-only pages — writes no-op. mprotect re-asserts the perm. */
    if (mprotect( tmp, size, PROT_READ | PROT_WRITE ) == -1)
        ws_log("[ios] set_session_mapping: mprotect RW failed errno=%d", errno);
#endif
...
#ifdef WINE_IOS
    /* See set_session_mapping comment — iOS-specific mmap PROT_WRITE
     * silently downgraded; mprotect required to make writes visible. */
    if (mprotect( tmp, new_size - old_size, PROT_READ | PROT_WRITE ) == -1) {
        ws_log("[ios] grow_session_mapping: mprotect RW failed errno=%d", errno);
    }
#endif
