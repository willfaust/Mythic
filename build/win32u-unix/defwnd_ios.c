...
#ifdef WINE_IOS
    /* iOS: sysparams cache is not populated (no driver, no
     * UpdateDisplayDevices). Defaults from get_entry are zero-init
     * structs that produce nonsensical inflated rects (cy in millions).
     * Hard-code sane Windows defaults so AdjustWindowRectEx returns
     * a usable rect — games then create reasonably-sized windows
     * even without a real driver. */
    if (ncm.iCaptionHeight <= 0 || ncm.iCaptionHeight > 200) {
        ncm.iBorderWidth        = 1;
        ncm.iPaddedBorderWidth  = 4;
        ncm.iScrollWidth        = 17;
        ncm.iScrollHeight       = 17;
        ncm.iCaptionWidth       = 22;
        ncm.iCaptionHeight      = 22;
        ncm.iSmCaptionWidth     = 22;
        ncm.iSmCaptionHeight    = 16;
        ncm.iMenuWidth          = 18;
        ncm.iMenuHeight         = 19;
    }
#endif
