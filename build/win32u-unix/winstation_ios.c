...
{
#ifdef WINE_IOS
    /* iOS: pretend we're a service so get_desktop_window() takes the
     * force=1 server path and avoids spawning explorer.exe (our broken
     * thread-based child-process path SEGVs on entry to PE child code). */
    return TRUE;
#else
