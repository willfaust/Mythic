...
#ifdef WINE_IOS
    os_log(OS_LOG_DEFAULT, "[Wine] add_dynamic_env: build_dir=%{public}s data_dir=%{public}s", build_dir ? build_dir : "(null)", data_dir ? data_dir : "(null)");
#endif
...
#ifdef WINE_IOS
    os_log(OS_LOG_DEFAULT, "[Wine] add_dynamic_env: nt_file_names done");
#endif

    append_envW( env, pos, size, "WINEBUILDDIR", nt_build_dir );
    append_envW( env, pos, size, "WINEDATADIR", nt_data_dir );
#ifdef WINE_IOS
    os_log(OS_LOG_DEFAULT, "[Wine] add_dynamic_env: WINEHOMEDIR home_dir=%{public}s", home_dir ? home_dir : "(null)");
#endif
    add_path_var( env, pos, size, "WINEHOMEDIR", home_dir );
#ifdef WINE_IOS
    os_log(OS_LOG_DEFAULT, "[Wine] add_dynamic_env: WINECONFIGDIR config_dir=%{public}s", config_dir ? config_dir : "(null)");
#endif
    add_path_var( env, pos, size, "WINECONFIGDIR", config_dir );
#ifdef WINE_IOS
    os_log(OS_LOG_DEFAULT, "[Wine] add_dynamic_env: WINELOADER wineloader=%{public}s", wineloader ? wineloader : "(null)");
#endif
    add_path_var( env, pos, size, "WINELOADER", wineloader );
#ifdef WINE_IOS
    os_log(OS_LOG_DEFAULT, "[Wine] add_dynamic_env: dll_paths loop...");
#endif
    for (i = 0; dll_paths[i]; i++)
    {
#ifdef WINE_IOS
        os_log(OS_LOG_DEFAULT, "[Wine] add_dynamic_env: dll_path[%u]=%{public}s", i, dll_paths[i]);
#endif
...
#ifdef WINE_IOS
    os_log(OS_LOG_DEFAULT, "[Wine] add_dynamic_env: add_system_dll_path_var...");
#endif
    add_system_dll_path_var( env, pos, size );
#ifdef WINE_IOS
    os_log(OS_LOG_DEFAULT, "[Wine] add_dynamic_env: remaining env vars...");
#endif
...
#ifdef WINE_IOS
    os_log(OS_LOG_DEFAULT, "[Wine] add_dynamic_env: DONE");
#endif
}
...
#ifdef WINE_IOS
    os_log(OS_LOG_DEFAULT, "[Wine] add_registry_environment: entering");
#endif
    InitializeObjectAttributes( &attr, &nameW, 0, 0, NULL );
    init_unicode_string( &nameW, syskeyW );
#ifdef WINE_IOS
    os_log(OS_LOG_DEFAULT, "[Wine] add_registry_environment: NtOpenKey(syskey)...");
#endif
    if (!NtOpenKey( &key, KEY_READ, &attr ))
    {
#ifdef WINE_IOS
        os_log(OS_LOG_DEFAULT, "[Wine] add_registry_environment: syskey opened, reading vars...");
#endif
        add_registry_variables( env, pos, size, key );
        NtClose( key );
    }
#ifdef WINE_IOS
    else os_log(OS_LOG_DEFAULT, "[Wine] add_registry_environment: syskey not found (OK for fresh prefix)");
    os_log(OS_LOG_DEFAULT, "[Wine] add_registry_environment: open_hkcu_key(Environment)...");
#endif
...
#ifdef WINE_IOS
    else os_log(OS_LOG_DEFAULT, "[Wine] add_registry_environment: HKCU\\Environment not found (OK)");
    os_log(OS_LOG_DEFAULT, "[Wine] add_registry_environment: open_hkcu_key(Volatile Environment)...");
#endif
...
#ifdef WINE_IOS
    else os_log(OS_LOG_DEFAULT, "[Wine] add_registry_environment: HKCU\\Volatile Environment not found (OK)");
#endif
...
static RTL_USER_PROCESS_PARAMETERS *build_initial_params( void **module )
{
...
#ifdef WINE_IOS
    os_log(OS_LOG_DEFAULT, "[Wine] build_initial_params: get_initial_environment...");
#endif
...
#ifdef WINE_IOS
    os_log(OS_LOG_DEFAULT, "[Wine] build_initial_params: add_dynamic_environment...");
#endif
    add_dynamic_environment( &env, &env_pos, &env_size );
#ifdef WINE_IOS
    os_log(OS_LOG_DEFAULT, "[Wine] build_initial_params: add_registry_environment...");
#endif
    ...
    env[env_pos] = 0;
#ifdef WINE_IOS
    os_log(OS_LOG_DEFAULT, "[Wine] build_initial_params: run_wineboot...");
#endif
    ...
    free( path );
#ifdef WINE_IOS
    /* On iOS, wineboot never runs (no fork/exec), so we're permanently in bootstrap mode.
       Keep WINEBOOTSTRAPMODE set so the PE-side can find builtin DLLs. */
    is_prefix_bootstrap = TRUE;
    free( bootstrap );
#else
...
#endif
#ifdef WINE_IOS
    os_log(OS_LOG_DEFAULT, "[Wine] build_initial_params: add_registry_environment (reload)...");
#endif
    add_registry_environment( &env, &env_pos, &env_size );
    env[env_pos++] = 0;

#ifdef WINE_IOS
    os_log(OS_LOG_DEFAULT, "[Wine] build_initial_params: get_full_path(%s)...", main_argv[1] ? main_argv[1] : "(null)");
#endif
    get_full_path( main_argv[1], curdir, &nt_name );
#ifdef WINE_IOS
    os_log(OS_LOG_DEFAULT, "[Wine] build_initial_params: load_main_exe...");
#endif
...
#ifdef WINE_IOS
    os_log(OS_LOG_DEFAULT, "[Wine] init_startup_info: startup_info_size=%lu", (unsigned long)startup_info_size);
#endif
    if (!startup_info_size)
    {
#ifdef WINE_IOS
        os_log(OS_LOG_DEFAULT, "[Wine] init_startup_info: calling build_initial_params...");
#endif
        params = build_initial_params( &module );
#ifdef WINE_IOS
        os_log(OS_LOG_DEFAULT, "[Wine] init_startup_info: build_initial_params returned, calling init_peb...");
#endif
        init_peb( params, module );
#ifdef WINE_IOS
        os_log(OS_LOG_DEFAULT, "[Wine] init_startup_info: done!");
#endif
        return;
    }
